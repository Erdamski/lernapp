  ...
  return (
    <div className="w-full h-full flex flex-col items-center p-6">
      <header className="w-full max-w-4xl flex items-center justify-between mb-2">
        ...
        <IconButton onClick={() => audio.speak(`${current.visible} plus wie viel ist ${current.total}?`)} aria-label="Vorlesen">
      ...
      <div className="flex-1 w-full flex flex-col items-center justify-center gap-10">
        ...
        <div className="flex justify-center gap-5 mt-4">
          {current.options.map((opt) => {
            const isCorrect = feedback === 'correct' && opt === current.missing;
            const reveal = feedback === 'wrong' && opt === current.missing;
            return (
              <button
                key={opt}
